...
- � **回答コピー** — ワンクリックでクリップボードにコピー
- 📤 **チャットエクスポート** — 会話を Markdown ファイルとしてダウンロード
- 🔎 **ドキュメント検索フィルタ** — 管理画面でファイル名を絞り込み
- ⏱️ **レート制限** — ユーザーごとの質問回数制限（設定可能）
- 📊 **利用ログ** — 管理画面で質問履歴・統計を閲覧
...
| `GET` | `/api/sessions/{id}/export` | セッションを Markdown でエクスポート |
...
| `GET` | `/api/usage-logs` | 利用ログ一覧（ページネーション対応） |
...
| `RATE_LIMIT_PER_MINUTE` | `20` | ユーザーあたりの1分間の質問回数上限 |
...
│   ├── rate_limit.py        # レート制限（ユーザーごと）
│   ├── audit_log.py         # 利用ログ (SQLite)
...
| **96GB** (RTX PRO 6000 Blackwell) | `qwen2.5:72b` | 45 GB | Apache 2.0 | 72B フル精度。量子化なしで最高品質 |
...
> - **Gemma**: 商用利用可。ただし出力を競合モデルの訓練に使用することを禁止する条項あり  
> - `llama3.1` (Llama Community License) は月間7億ユーザー超で Meta への別途ライセンス申請が必要なため、本一覧からは除外しています
### Embedding モデルの差し替え
...
## Linux 本番デプロイ（Docker + Caddy）

閉域網や `docker exec ollama ollama pull` が使えない環境向けの構成です。  
サンプルファイル: [`docker-compose.linux.yml`](docker-compose.linux.yml) / [`Caddyfile.linux`](Caddyfile.linux)

### 構成概要

```
クライアント → Caddy (:80) → rag-app (:8000) → ollama (:11434)
                                    ↓
                              ChromaDB + SQLite (/data)
                              モデル (/data/LLM)
```

### 1. モデルの事前配置

インターネット接続のある別マシンで Ollama を使いモデルを取得し、APPSERVER へ転送します。
...
sudo tar xzf ollama-models.tar.gz -C /data/LLM

# 確認（以下のような構成になっていること）
ls /data/LLM/models/manifests/registry.ollama.ai/library/
...
新しいモデルを追加する場合も同じ手順で転送します。

```bash
# --- 接続可能なマシンで ---
ollama pull qwen2.5:14b
tar czf qwen2.5-14b.tar.gz -C ~/.ollama/models/manifests/registry.ollama.ai/library qwen2.5

# 対応する blob も転送が必要
# blobs ディレクトリから該当 sha256 ファイルをコピー

# --- APPSERVER で ---
# ファイルを /data/LLM/models/ 以下の対応パスに配置
docker restart ollama
docker exec ollama ollama list
```

> **ヒント**: モデル全体をまるごと転送するのが最も確実です。  
> `tar czf ollama-models.tar.gz -C ~/.ollama .` → APPSERVER の `/data/LLM` に展開。
